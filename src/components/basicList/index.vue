    ...
    <div class="basic-list" v-loading="loading">
        <div class="basic-list__hd f-cb" v-if="showHead">
            ...
            <div class="basic-list__hd-col--r f-fr">
                ...
                <!--兼容老代码-->
                <a class="el-icon-setting basic-list__field-set-btn" @click="extDlgVis" v-if="showExtSet"></a>
                <!--封装 组件内置-->
                <a class="el-icon-setting basic-list__field-set-btn" @click="updateExtendSetDlgVis(true)" v-if="showExtendSet"></a>
            </div>
        </div>
        <el-form :disabled="isTabDisabel">
            <el-table border
                      ...
                      :cell-class-name="cellClassNames.join(' ')"
                      :row-class-name="rowClassNames.join(' ')"
                      style="width: 100%">
                <div slot="">
                ...
                <el-table-column align="center"
                                 ...
                                 disabled="true"
                                 v-if="showSelectCol && (selectType === 'multiple' || selectType === 'singleMult')"/>
                <!--单选-->
                <el-table-column align="center"
                                 fixed="left"
                                 :width="50"
                ...
                <el-table-column type="index"
                                 :width="indexColWidth"
                                 :fixed="indexLocked ? 'left': undefined"
                                 :label="indexColLabel"
                                 :index="getIndex"
                                 :align="indexAlign"
                ...
                <template v-for="(item, index) of colFields">
                    <el-table-column v-if="!item.render && !item.hidden"
                                     :key="`${item.key}-${index}`"
                    ...
                    <el-table-column v-else-if="!item.hidden"
                                     :key="`${item.key}-${index}`"
                                     :class-name="item.classNames ? item.classNames.join(' ') : null"
                                     :prop="item.key"
                                     :label="item.name"
                                     ...
                                     :render-header="item.hdRender">
                        <template slot-scope="scope">
                            <table-column :index="scope.$index"
                                          :row="scope.row"
                                          :col="scope.column"
                                          :field="item"
                                          :render="item.render"
        ...
        <column-set-dlg :visible="colSetDlgVis"
