        ...
        return (
            <div class={['basic-search', `basic-search--${this.size}`, this.closed ? 'basic-search--close' : 'basic-search--open']}>
                <div class="basic-search__bd">
                    ...
                    <div class="col-r">
                        {this.showQueryBtn && <el-button type="primary" size="small" onClick={this.handleSearch}>查询</el-button>}
                        {this.showResetBtn && <el-button size="small" onClick={this.handleReset}>重置</el-button>}
                        {this.showMore && <a title="展开更多" class="toggle-more-btn" onClick={this.toggleMore}>{this.closed ? '展开查询' : '收起查询'}</a>}
                    </div>
                </div>
            </div>
        )
