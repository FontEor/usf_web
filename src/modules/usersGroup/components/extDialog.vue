...
export default {
    ...
    return {
      ...
      fields: [
        ...
        },
        {
          ...
          name: "字段名称",
          render: (h, { row }) => (
            <el-input v-model_trim={row.extName} size="small" />
          ),
        },
        {
          key: "isEnum",
          ...
          render: (h, { row }) => (
            <el-checkbox
              v-model={row.isEnum}
              onChange={(val) => this.mutexEnumBig(['isBigField'], val, row)}
              true-label={1}
              false-label={0}
            />
          ),
        ...
        },
        {
          ...
          render: (h, { row }) => (
            <el-checkbox
              v-model={row.isBigField}
              onChange={(val) => this.mutexEnumBig(['isEnum'], val, row)}
              true-label={1}
              false-label={0}
            />
          ),
  ...
  watch: {
    visible(nVis) {
      if (nVis) {
        this.queryGroupExtendList();
      }
    },
  },
  methods: {
    getDefaultList() {
      const listLen = Array(10).fill(0);
    ...
    async saveHandler() {
      ...
      // 提交的时候在这 api的接口
      const { code, msg } = await api.users.extGroupAdd({ data: list });
      if (code === 200) {
    ...
    checkParams() {
      for (let i = 0, list = this.list, len = list.length; i < len; i++) {
        ...
        if (row.isEnum === 1 && !_.isEmpty(row.enumValue)) {
          ...
          if(row.isEnum && row.isBigField) {
            this.$message.error(
              `字段序号${row.extNo}不允许即是枚举，还是大字段`
            );
            return 
          }
        }
