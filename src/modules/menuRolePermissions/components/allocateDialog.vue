...
export default {
    ...
    methods: {
        ...
        queryList () {
            ...
            api.menuRolePermissions.queryMenuRoleUserListByPage(params).then( ({code, data}) => {
                if (code === 200) {
                    this.list = data.rows || []
        ...
        queryToEnd () {
            ...
            data.appCode = this.$parent.$refs.exactSearch.appCode
            if(!data.appCode) {
                try {
                    data.appCode = this.$parent.$refs.exactSearch.query.appCode
                } catch (error) {}
            }
        ...
        toEnd () {
            // 选择的角色 this.selectRole  分配用户 this.selRows
            let { roleCode, appCode = this.$parent.$refs.exactSearch.appCode } = this.selectRole || {}
            const select = []
            this.selRows.forEach( item => { select.push(item.id) } )
            const all = []
            this.list.forEach( item => { all.push(item.id) } )
            const params = { data: { roleCode, tenementCode: this.tenementCode, select, all, appCode } }
            return params
        },
        // 分配数据角色管理权限
        handleSave (params) {
            api.menuRolePermissions.grant(params).then( ({code, data, msg}) => {
                if (code === 200) {
