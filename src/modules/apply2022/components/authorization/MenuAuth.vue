<template>
  <div class="apply-2022-auth-choose-c">
    <div>
      <h3>
        选择菜单角色{{ applyType === 'applyAdmin' ? '管理权限' : '' }}
        <span v-if="menuRoleHint">
          <a :href="menuRoleHint" target="_blank">如何选择权限?</a>
        </span>
        <span v-else>
      ...
      <vxe-table stripe border="inner" ref="table" :row-config="{ isHover: true }" :column-config="{resizable: true}" :data="list" max-height="550">
        <vxe-column field="appName" title="应用"></vxe-column>
...
import MenuRoleDialog from "../dialog/MenuRoleDialog.vue";
...
export default {
  ...
  computed: {
    ...
    menuRoleHint() {
      return store.state.configInfo && store.state.configInfo.menuRoleHint
    },
  },
  mounted() { },
  methods: {
