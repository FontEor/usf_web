<template>
  <div class="apply-2022-auth-choose-c">
    <div>
      <h3>
        选择USF菜单角色
        <span v-if="usfRoleHint">
          <a :href="usfRoleHint" target="_blank">如何选择权限?</a>
        </span>
        <span v-else>
      ...
      >
        ...
        <vxe-column field="isOwnRole" title="是否已分配" show-overflow>
          <template #default="{ row }">
            <span :style="{
...
export default {
  ...
  computed: {
    ...
    usfRoleHint() {
      return store.state.configInfo && store.state.configInfo.usfRoleHint
    },
  },
  data() {
    return {
