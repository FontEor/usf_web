<template>
  <div class="apply-2022-auth-choose-c">
    <div>
      <h3>
        选择数据角色{{ applyType === 'applyAdmin' ? '管理权限' : '' }}
        <span v-if="dataRoleHint">
          <a :href="dataRoleHint" target="_blank">如何选择权限?</a>
        </span>
        <span v-else>
      ...
      <vxe-table
        ...
        max-height="550"
      >
        <vxe-column field="roleName" title="数据角色名称"></vxe-column>
        <vxe-column field="roleCode" title="数据角色编码"></vxe-column>
        <vxe-column v-if="formData.applyType === 'self'" field="isOwnRole" title="是否已分配" show-overflow>
...
import DataRoleDialog from "../dialog/DataRoleDialog.vue";
...
export default {
  ...
  computed: {
    ...
    dataRoleHint() {
      return store.state.configInfo && store.state.configInfo.dataRoleHint
    },
  },
  mounted() {},
  methods: {
