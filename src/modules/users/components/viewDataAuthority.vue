      ...
      <div class="entity-dlg__block">
        <div class="entity-dlg__block-bd formFix">
          <el-row :gutter="0">
            ...
            <el-form-item
              :label="item.name"
              v-for="(item, index) in dataExtInfoList"
              :key="index"
            >
      ...
      >
        <template v-slot:action="row">
          <el-popover placement="top" trigger="click">
            <div>
              <el-alert
                title="用户从以下渠道获得了该数据资源的权限,如果想要去除用户对于该数据资源的权限,需要将用户从以下渠道中全部移除"
                type="info"
                :closable="false"
  ...
  methods: {
    ...
    dtTypeChange(code, flag) {
      ...
      this.$parent.updateState({
        entity: { searchList: this.searchList, typeId: getitem[0].id },
      });
      this.pageIndex = 0;
      this.$parent.updateState({ adminPageIndex: 0 });
      this.$parent.dispatch("queryUserBaseData").then((_data) => {
        // 判断是否有数据
        ...
        this.dataExtInfoList.forEach((ev) => {
          ...
          if (ev.type === "enums") {
            col.render = (h, { row }) => {
              const itemEnum = this.extendFieldEnumMap[col.key].find(
                (item) => item.value === row[col.key]
              );
    ...
    watchData({ row }) {
      ...
      api.users.queryUserBaseDataAssignSource({ data: params }).then((res) => {
        if (res.code == 200) {
    ...
    pageIndexChange(_index) {
      ...
      // 查询列表
      this.$parent.dispatch("queryUserBaseData");
    },
    // 切换每页显示条数
