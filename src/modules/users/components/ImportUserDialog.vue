<template>
    ...
    >
        <!-- <template v-if="showStart">
            ...
            <div class="showStart">
                <div class="showStart__title">请选择需要导入的方式:</div>
                <el-row>
                    <el-col :span="12">

                        <div class="leftBox">

                            <div class="boxTitle">快速导入</div>
                            <div class="boxMain"> 导入数量小于1万条</div>
                            <el-button class="leftButton" size="mini" type="primary" @click="linkToLittle">选择
                            </el-button>
                        </div>
                    </el-col>
                    <el-col :span="12">
                        <div class="rightBox">
                            <div class="boxTitle">大批量导入</div>
                            <div class="boxMain">导入数据量1万条以上<br/>注：单个文件不能超过20M</div>
                            <el-button class="rightButton" size="mini" type="primary" @click="linkToMuch">选择</el-button>
                        </div>
                    </el-col>
                </el-row>
            </div>
        ...
        <template v-if="showLittleMould">
            ...
            <div class="showLittleMould">
                <div class="title">下载导入模板Excel文件，填写Excel中的字段，保存后上传Excel文件。</div>
                <div class="explain">模板填写事项：<br/>
                ...
                <el-row>
                    <el-col :span="18">
                        <el-button class="leftButton" size="mini" type="primary" @click="submitDown">下载导入模板
                        </el-button>
                    </el-col>
                    <el-col :span="4">
                        <el-button class="rightButton" size="mini" @click="linkToLittleInit">我已填好，下一步
                        </el-button>
                    </el-col>
                </el-row>
            </div>
        ...
        <template v-if="showLittleInit">
            ...
            <div class="showLittleInit">
                ...
                <el-upload
                    ...
                    :action="`${VUE_APP_API_PREFIX}${this.uploadData.uploadUrl}?tenementCode=${$store.state.app.tenementCode}&isBigFile=0`"
                    :auto-upload="true"
                ...
                <div class="main">注：单次导入最多不超过1万条数，超出请分批次上传或选择大批量导入</div>
        ...
        <!--        showLittlePending有一个进度条，1min到99，成功之后跳转页面-->
            ...
            <div class="bigEnd">
                <p class="word">文件正在导入中，当前导入条数过多，您可以离开界面去处理其他任务，<br/>导入完成后结果将通过咚咚/邮件下发，请注意查收。</p>
                <el-button class="closeBtn" size="mini" type="primary" @click="handleClose">关闭</el-button>
            </div>
        </template>
        ...
        <template v-if="showLittleEnd">
            ...
            <div class="end">
                <p class="word">导入完成，<i class="upload-result__disc-success">{{importResult.successCounts}}</i>条数据导入成功，<i
                    class="upload-result__disc-fail">{{importResult.failureCounts}}</i>条数据导入失败</p>
                <el-button class="loadBtn" size="mini" type="primary" @click="downImportResult">下载导入结果</el-button>
            </div>
        </template>
        <template v-if="showLittleEndError">
            ...
            <div class="error">
                ...
                <p class="word"><i class="el-icon-warning-outline" style="color: #ffb600"></i>{{updateResultMag}}</p>
                <el-button class="loadBtn" size="mini" type="primary" @click="linkToLittleInit">重新上传</el-button>
            </div>
        </template>
        <!-- <template v-if="showLittleToMuch">
            <el-steps :active="active">
                <el-step title="选择导入方式"></el-step>
                <el-step title="下载导入模板"></el-step>
                <el-step title="上传模板"></el-step>
            ...
            <div class="error">
                <p class="word"><i class="el-icon-warning-outline" style="color: #ffb600"></i>{{updateResultMag}}</p>
                <el-button class="loadBtn" size="mini" type="primary" @click="littleToMuch">大批量导入</el-button>
            </div>
        </template>
        <template v-if="showMuchMould">
            <el-steps :active="active2">
                <el-step title="选择导入方式"></el-step>
                <el-step title="下载导入模板"></el-step>
                <el-step title="上传模板"></el-step>
            ...
            <div class="showLittleMould">
                ...
                <el-row>
                    <el-col :span="10">
                        <el-button class="middleButton" size="mini" type="primary" @click="submitDown">下载导入模板
                        </el-button>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="18">
                        <el-button class="leftButton" size="mini" @click="backToMenu">上一步</el-button>
                    </el-col>
                    <el-col :span="4">

                        <el-button class="rightButton" size="mini" @click="linkToMuchInit">我已填好，下一步
                        </el-button>
                    </el-col>
                </el-row>
            </div>
        </template>
        <template v-if="showMuchInit">
            <el-steps :active="active2">
                <el-step title="选择导入方式"></el-step>
                <el-step title="下载导入模板"></el-step>
                <el-step title="上传模板"></el-step>
            ...
            <div class="showLittleInit">
                <el-upload
                    ...
                    drag
                    :headers="{'traceId':Date.now()}"
                    :action="`${VUE_APP_API_PREFIX}${this.uploadData.uploadUrl}?tenementCode=${$store.state.app.tenementCode}&isBigFile=1`"
                    :auto-upload="true"
                    :before-upload="beforeAvatarUploadMuch"
                    :on-success="handleSuccessMuch"
                    :on-progress="handleProgressMuch"
                    :on-error="handleErrorMuch"
                    multiple>
                    <i class="el-icon-upload"></i>
                    <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
                </el-upload>
                <div class="main">注：单次文件不能超过20M，超出请分批次上传</div>
                <el-button class="backButton" size="mini" @click="linkToMuch">上一步</el-button>
            </div>
        </template>
        <template v-if="showMuchPending">
            <el-steps :active="active2">
        ...
        <template v-if="showMuchEndError">
            <el-steps :active="active2">
                <el-step title="选择导入方式"></el-step>
                <el-step title="下载导入模板"></el-step>
                <el-step title="上传模板"></el-step>
            ...
            <div class="error">
                <p class="word"><i class="el-icon-warning-outline" style="color: #ffb600"></i>{{updateResultMag}}
                </p>
                <el-button class="loadBtn" size="mini" type="primary" @click="linkToMuchInit">重新上传</el-button>
            </div>
        </template> -->
...
<style lang="scss" scoped>
    ...
    .showLittleMould {
        ...
        .explain {
            ...
            height: 110px;padding-left: 16px;
    ...
    .upload-pending__disc, .upload-result__disc {
        margin: 10px 0;
    }
    ...
    .upload-result__disc-success, .upload-result__disc-fail {
        font-style: normal;
    }
