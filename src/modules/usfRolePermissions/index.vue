...
export default {
  ...
  methods: {
    ...
    importRole(buttonName) {
      ...
      this.uploadData = {
        ...
        templateType: "role",
        name: "roleFile",
        enabled: this.buttonData('CDJSDR2').has && this.showExportButton
      };
    ...
    importRoleResource(buttonName) {
      ...
      this.uploadData = {
        ...
        enabled: this.buttonData('DRCDJSZY2').has && this.showExportButton
      };
      this.importDialogTitle = buttonName;
    },
    // 获取菜单
    ...
    async queryExtendList() {
      ...
      const { code, msg, data } = await api.rolePermissions.queryRoleExtList(params);
      if (code === 200) {
    ...
    setListExtendFields(extendList) {
      const showExtendFields = extendList.filter((item) => item.isEffective === 1);
      ...
      const extendFields = showExtendFields.map((item) => {
        const col = { key: `ext${item.extNo}`, name: item.extName, hidden: false };
        if (item.isEnum === 1) {
          col.render = (h, { row }) => {
            const enums = jt.fn.generateEnums(item.enumValue, ",");
            const enumValue = enums.find((subItem) => subItem.value === row[col.key]);
    ...
    */
      ...
      axios({
        ...
        .then((resolve) => {
          if (
            ...
            this.$message({
              message: "该角色已经分配给用户/用户组使用，请先解除关联关系再删除",
              type: "error",
              center: true,
            });
          } else if (
            ...
            this.$message({
              message: "该角色已经分配给用户/用户组/机构使用，请先解除关联关系再删除",
              type: "error",
              center: true,
            });
          } else if (
            ...
            this.$message({
              message: "该角色已经分配给用户组/机构使用，请先解除关联关系再删除",
              type: "error",
              center: true,
            });
          } else if (
    ...
    deleteSure(params) {
      ...
      this.$confirm("确认删除选中的数据资源吗", "操作提示")
        .then(() => {
  ...
  updated() {},
  computed: {
    ...mapState("usfRolePermissions", ["query"]),
    ...
    "state.query": {
      ...
      handler: function (newVal) {
        if (newVal.appCode) {
          ...
          api.rolePermissions.queryRoleExtList(params).then(({ code, data, message }) => {
            if (code === 200) {
