...
export default {
  ...
  methods: {
    ...
    async saveHandler() {
      ...
      const args = { appCode: "usf", data: JSON.stringify(list) };

      const { code, msg } = await api.rolePermissions.extRoleAddOrEdit({
        data: args,
      });
      if (code === 200) {
    ...
    async queryExtendList() {
      ...
      const params = {
        data: { tenementCode: this.tenementCode, appCode: "usf" },
      };
